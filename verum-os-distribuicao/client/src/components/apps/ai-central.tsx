  ...
  return (
    <div className="h-full overflow-auto p-6 space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <GlassPanel 
          className={`p-4 cursor-pointer border-2 transition-all ${
            activeAI === 'chatgpt' ? 'border-green-400 bg-green-900/20' : 'border-verum-border'
          }`}
          onClick={() => switchAI('chatgpt')}
        >
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-lg font-semibold text-green-400">ChatGPT-4o</h3>
            <Badge variant="outline" className={aiStats.chatgpt.status === 'active' ? 'text-green-400 border-green-400' : 'text-gray-400 border-gray-400'}>
              {aiStats.chatgpt.status.toUpperCase()}
            </Badge>
          </div>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-400">Tokens Processados:</span>
              <span className="text-green-400">{aiStats.chatgpt.tokens.toLocaleString()}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Tempo de Resposta:</span>
              <span className="text-green-400">{aiStats.chatgpt.responseTime}ms</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Inteligência:</span>
              <span className="text-verum-orange">{aiStats.chatgpt.intelligence}%</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-400">Context Window:</span>
