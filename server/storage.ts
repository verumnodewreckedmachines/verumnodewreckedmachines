...
export interface IStorage {
  ...
  // Application methods
  getAllApplications(limit?: number): Promise<Application[]>;
  ...
  // Document methods
  getAllDocuments(userId?: number, type?: string, limit?: number): Promise<Document[]>;
...
export class MemStorage implements IStorage {
  ...
  // Application methods
  async getAllApplications(limit: number = 100): Promise<Application[]> {
  ...
  // Document methods
  async getAllDocuments(userId?: number, type?: string, limit: number = 100): Promise<Document[]> {
...
export class DatabaseStorage implements IStorage {
  ...
  // Application methods
  async getAllApplications(limit: number = 100): Promise<Application[]> {
    return await db.select().from(applications).orderBy(desc(applications.downloads)).limit(limit);
  }
  ...
  async getAllDocuments(userId?: number, type?: string, limit: number = 100): Promise<Document[]> {
    if (userId && type) {
      return await db.select().from(documents)
        ...
        .orderBy(desc(documents.lastModified)).limit(limit);
    } else if (userId) {
      return await db.select().from(documents)
    ...
    return await db.select().from(documents).orderBy(desc(documents.lastModified)).limit(limit);
  }
